...
import { vOverWSHandler } from './protocols/vless';
...
import { getXxrCustomConfigs, getXxrWarpConfigs } from './cores-configs/xray';
import { getSBCustomConfig, getSBWConfig } from './cores-configs/sing-box';
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {            
                switch (globalThis.pathName) {                    
                    ...
                    case `/sub/${globalThis.userID}`:
                        if (globalThis.client === 'sfa') return await getSBCustomConfig(request, env, false);
                        if (globalThis.client === 'clash') return await getClashNormalConfig(request, env);
                        if (globalThis.client === 'xray') return await getXxrCustomConfigs(request, env, false);
                        return await getNormalConfigs(request, env);                        

                    case `/fragsub/${globalThis.userID}`:
                        return globalThis.client === 'hiddify'
                            ? await getSBCustomConfig(request, env, true)
                            : await getXxrCustomConfigs(request, env, true);

                    case `/warpsub/${globalThis.userID}`:
                        if (globalThis.client === 'clash') return await getClashWarpConfig(request, env);   
                        if (globalThis.client === 'singbox' || globalThis.client === 'hiddify') return await getSBWConfig(request, env, globalThis.client);
                        return await getXxrWarpConfigs(request, env, globalThis.client);
